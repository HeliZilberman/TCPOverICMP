...
class ClientManager:
    ...
    async def write_to_client(self, client_id: int, seq: int, data: bytes):
        """
        ...
        try:
            await self.clients[client_id].session.write(seq, data)
    ...
    async def read_from_client(self, client_id: int):
        ...
        try:
            while True:
                ...
                await self.incoming_tcp_packets.put((data, client.client_id, next(client.seq)))
