...
class ICMPTunnelPacket:
    ...
    Handles optional fields gracefully.
    """
    # Struct format includes destination_host as a fixed 4-byte field for IPv4
    TUNNEL_STRUCT = struct.Struct('>IIHHI4s')  # session_id, seq, action, direction, port, destination_host (4 bytes)

    def __init__(self, session_id, action, direction, seq=0, destination_host='0.0.0.0', port=0, payload=b''):
        """
        Initialize a ICMPTunnelPacket.
        :param session_id: Mandatory. ID of the session.
        :param action: Mandatory. Instance of Action Enum.
        :param direction: Mandatory. Instance of Direction Enum.
        :param seq: Optional. Sequence number.
        :param destination_host: Mandatory. IPv4 address as a string.
        :param port: Optional. Destination port as an integer.
        :param payload: Optional. Payload as bytes.
        """
        self.session_id = session_id
    ...
    def serialize(self):
        ...
        """
        try:
            ip_binary = socket.inet_aton(self.destination_host)  # Convert to 4-byte binary IPv4 address
        except socket.error:
            raise ValueError(f"Invalid IPv4 address for destination host: {self.destination_host}")
    ...
    def deserialize(cls, packet):
        ...
        header_size = cls.TUNNEL_STRUCT.size
        header = packet[:header_size]
        session_id, seq, action, direction, port, ip_binary = cls.TUNNEL_STRUCT.unpack(header)

        action = Action(action)
        direction = Direction(direction)
        destination_host = socket.inet_ntoa(ip_binary)  # Convert binary IP back to string
        payload = packet[header_size:]  # Extract the payload
